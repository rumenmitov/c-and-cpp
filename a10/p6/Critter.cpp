...
// Critter classes with added thirst level
Critter::Critter() {
    // default Critter
...
Critter::Critter(const std::string& newname, int hunger, 
    ...
    this->thirst = convert(hunger);

    cout << "Hello from Case 3 Constructor 👋 (using height of " 
        << this->height << ")\n";
}

Critter::Critter(const std::string& newname, int hunger, int boredom, 
        double height, double thirst) : name(newname) {
    ...
    cout << "Hello from Case 4 Constructor 👋" << endl;
}
...
// modifed print method
void Critter::print() {
	cout << "I am " << this->name << ".\nMy hunger level is " 
        << convert(this->hunger) << ".\nMy thirst level is " << convert(thirst)
